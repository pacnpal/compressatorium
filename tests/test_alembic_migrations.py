...
Four behaviours under test, all load-bearing for a safe upgrade:

* **I1** — Fresh DB: ``apply_migrations`` runs ``upgrade head`` and
  populates ``alembic_version`` at ``0001``.
* **I2** — Pre-Alembic DB (baseline schema exists, no
  ``alembic_version``): ``apply_migrations`` *stamps* head without
  running any DDL, and existing rows survive.
