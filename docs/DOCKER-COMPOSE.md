...
| `COMPRESSATORIUM_<TOOL>_*` | *(shared default)* | Optional per-tool overrides (`<TOOL>` = `CHDMAN`, `DOLPHIN_TOOL`, `NSZ`, `Z3DS`) that fall back to the shared `COMPRESSATORIUM_TOOL_*` values. |
